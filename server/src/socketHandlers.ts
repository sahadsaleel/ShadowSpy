...
import {
    ...
    startTimer, pauseTimer, resetGame, updateSettings, getRoom, closeRoom
} from './gameStore.js';
...
export const registerSocketHandlers = (io: Server) => {
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('join_room', ({ roomId, name }: { roomId: string, name: string }, callback) => {
            ...
            if (room) {
                ...
                if (callback) callback({ success: false, error: 'Room not found or game started' });
            }
        });
        ...
        socket.on('start_timer', ({ roomId }: { roomId: string }) => {
        ...
        socket.on('pause_timer', ({ roomId }: { roomId: string }) => {
