...
const rooms = {};
export const createRoom = (hostId, hostName) => {
...
export const updateSettings = (roomId, settings) => {
    const room = rooms[roomId];
    if (!room)
        return null;
...
export const pauseTimer = (roomId) => {
    const room = rooms[roomId];
    if (!room || !room.timerEndTime)
        return null;
    if (room.isPaused) {
        // Resume
